...
function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const toast = useCallback(
    ({ title, description, variant = "default", duration = 5000 }: ToastInput) => {
        ...
        <AnimatePresence initial={false}>
          {toasts.map((t) => (
            <motion.div
              ...
              className={cn(
                "pointer-events-auto relative flex items-start gap-3 rounded-lg border border-border bg-raised p-3.5 pr-9 shadow-xl shadow-black/40",
              )}
            >
